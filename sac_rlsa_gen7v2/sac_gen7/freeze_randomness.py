...
def freeze(freeze_b_path: Path, out_json: Path, out_indices: Path, salt: str) -> dict:
    ...
    indices = np.sort(
        rng.choice(freeze_b["population"], size=freeze_b["sample_size"], replace=False).astype("<u4")
    )
    out_indices.parent.mkdir(parents=True, exist_ok=True)
    indices.tofile(out_indices)
    record = {
        "schema": "SAC_GEN7_FREEZE_C_V2",
        "target_id": freeze_b["target_id"],
        "freeze_b_sha256": freeze_b["freeze_b_sha256"],
        "public_salt": salt,
        "seed_128": seed128,
        "sample_size": int(indices.size),
        "indices_sha256": sha256_file(out_indices),
...
def main() -> None:
    ...
    record = freeze(Path(args.freeze_b), Path(args.out_json), Path(args.out_indices), args.salt)
