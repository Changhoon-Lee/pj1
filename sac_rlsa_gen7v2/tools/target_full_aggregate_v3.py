...
def main() -> None:
    ...
    p.add_argument("--state3", required=True)
    p.add_argument("--ubuntu", required=True)
    p.add_argument("--macos", required=True)
    p.add_argument("--out-state4", required=True)
    p.add_argument("--out-state5", required=True)
    p.add_argument("--out-state6", required=True)
    p.add_argument("--out", required=True)
    args = p.parse_args()

    state3 = json.loads(Path(args.state3).read_text(encoding="utf-8"))
    verify(state3)
    ...
    ur = {int(r["seed"]): r for r in ubuntu["records"]}
    mr = {int(r["seed"]): r for r in macos["records"]}
    if set(ur) != set(mr):
        raise RuntimeError("full target set mismatch")
    ...
    summary = {
        "schema": "SAC_GEN7_CROSS_PLATFORM_FINAL_SUMMARY_V3",
        "protocol": "CANONICAL_ORACLE_AND_B7_COMPARATOR_SEPARATED",
        "ubuntu_receipt_digest": ubuntu["receipt_digest"],
        "macos_receipt_digest": macos["receipt_digest"],
        "checks": checks,
        "pass": passed,
    }
    ...
    state6 = transition(
        state5,
        6,
        "FINAL_HARD_GATES_ADJUDICATED_PASS" if passed else "FINAL_HARD_GATES_ADJUDICATED_FAIL",
    )
    write_json(Path(args.out_state6), state6)
    if not passed:
        raise RuntimeError(f"cross-platform final gate failed: {checks}")
